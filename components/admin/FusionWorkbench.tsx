...
export function FusionWorkbench() {
  ...
  return <div className="admin-fusion-page"><PageHeader eyebrow="Synthesis Workbench / 04" title="经验融合" actions={<SecondaryLink href="/admin/scenarios">返回任务</SecondaryLink>} />
    {error ? <div className="admin-demo-notice" role="alert"><div><strong>操作提示</strong><p>{error}</p></div></div> : null}
    <div className="admin-fusion-context"><div><span>当前任务</span><select className="admin-select" value={scenarioId} onChange={(event) => selectScenario(event.target.value)}>{scenarios.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div><div><span>可用访谈</span><strong>{String(interviews.length).padStart(2, "0")}</strong></div><div><span>已选来源</span><strong>{String(selected.length).padStart(2, "0")}</strong></div><div><span>状态</span><strong>待审核</strong></div></div>
