...
export function ScenarioDetail({ id }: { id: string }) {
  ...
  return <div className="admin-detail-page">
    ...
    <section className="admin-detail-content" role="tabpanel">
      {tab === "config" ? <div className="admin-detail-config"><div className="admin-card admin-config-overview"><div className="admin-card__header"><div><h2>任务信息</h2></div></div><div className="admin-card__body"><dl className="admin-meta-list"><div><dt>归属机构</dt><dd>{institutionName(scenario.institutionCode)} · {scenario.institutionCode ?? "历史未归类"}</dd></div><div><dt>希望萃取的经验</dt><dd>{scenario.objective}</dd></div><div><dt>参与者需要填写的信息</dt><dd className="admin-chip-list">{scenario.customFields.map((field) => <span className="admin-chip" key={field.id}>{field.fieldName}{field.required ? " · 必填" : ""}</span>)}</dd></div><div><dt>业务热词</dt><dd className="admin-chip-list">{scenario.keywords.length ? scenario.keywords.map((keyword) => <span className="admin-chip" key={keyword}>{keyword}</span>) : <span className="admin-meta-empty">未设置</span>}</dd></div></dl></div></div><div className="admin-detail-config__rail"><article className="admin-card"><div className="admin-card__header"><div><h2>案例设置</h2></div></div><div className="admin-card__body"><dl className="admin-meta-list"><div><dt>模拟案例生成</dt><dd>{scenario.caseTemplate.instruction}</dd></div><div><dt>核心萃取维度</dt><dd className="admin-chip-list">{dimensions.map((item) => <span className="admin-chip" key={item}>{item}</span>)}</dd></div><div><dt>参与者案例策略</dt><dd>{scenario.outputSchema.caseMode === "variant" ? "按指标组合生成多种案例" : "使用固定案例策略"}</dd></div></dl></div></article></div></div> : null}
