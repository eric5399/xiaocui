...
import type { ScenarioWithFields } from "@/lib/domain";
import { listAdminInterviews, listAdminScenarios, listAdminTasks, type AdminTask } from "./admin-data-client";
import { EmptyState, PageHeader, PrimaryLink, ProgressBar, StatusBadge } from "./ui";
...
export function ScenarioList() {
  ...
  const rows = useMemo(() => scenarios.map((scenario) => {
    ...
    return (status === "all" || row.status === status) && (!normalized || [row.scenario.name, row.scenario.topic, row.scenario.objective, row.task?.inviteCode].filter(Boolean).join(" ").toLocaleLowerCase("zh-CN").includes(normalized));
  }), [completedByTask, query, scenarios, status, tasks]);

  return <div className="admin-scenario-list">
    <PageHeader eyebrow="Scenario Library / 02" title="萃取任务" description="将一次业务调研拆为可管理的场景、访谈和经验产出。" actions={<PrimaryLink href="/admin/scenarios/create">新建任务</PrimaryLink>} />
    {error ? <div className="admin-demo-notice" role="alert"><div><strong>读取失败</strong><p>{error}</p></div></div> : null}
    <section className="admin-list-toolbar" aria-label="任务筛选"><label className="admin-search-field"><span aria-hidden="true" /><span className="sr-only">搜索任务</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索任务名、主题或邀请码" /></label><select className="admin-select admin-list-toolbar__select" aria-label="按状态筛选" value={status} onChange={(event) => setStatus(event.target.value as StatusFilter)}>{statusOptions.map((option) => <option value={option.value} key={option.value}>{option.label}</option>)}</select><span className="admin-list-toolbar__count">{String(rows.length).padStart(2, "0")} / {String(scenarios.length).padStart(2, "0")}</span></section>
    {loading ? <section className="admin-card"><EmptyState title="正在读取任务" description="正在通过机构权限加载真实任务数据。" /></section> : rows.length ? <section className="admin-scenario-stack" aria-live="polite"><div className="admin-scenario-stack__heading" aria-hidden="true"><span>任务 / 主题</span><span>采集进度</span><span>产出</span><span>状态 / 更新</span><span /></div>{rows.map(({ scenario, task, status: rowStatus, completed }) => { const total = task?.completedInterviewCount ?? 0; const completion = total ? Math.round(completed / total * 100) : 0; return <article className="admin-scenario-row" key={scenario.id}><div className="admin-scenario-row__title"><span className="admin-scenario-row__topic">{scenario.topic}</span><h2><Link href={`/admin/scenarios/${scenario.id}`}>{scenario.name}</Link></h2><p>{task ? `邀请码：${task.inviteCode}` : "尚未创建参与任务"}</p></div><div className="admin-scenario-row__progress"><div><strong>{completed}/{total || "—"}</strong><span>{total ? `${completion}%` : "未开始"}</span></div><div className="admin-progress" role="progressbar" aria-label={`${scenario.name} 采集进度`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={completion}><span style={{ width: `${completion}%` }} /></div><small>已完成访谈</small></div><div className="admin-scenario-row__output"><strong>—</strong><span>待融合生成</span></div><div className="admin-scenario-row__state"><StatusBadge status={rowStatus} /><small>{formatDate(task?.updatedAt ?? scenario.updatedAt)}</small></div><Link href={`/admin/scenarios/${scenario.id}`} className="admin-scenario-row__open" aria-label={`打开${scenario.name}`}>→</Link></article>; })}</section> : <section className="admin-card"><EmptyState title="没有找到匹配任务" description="调整筛选条件，或新建一个萃取任务。" action={<Link className="admin-button admin-button--secondary" href="/admin/scenarios/create">新建任务</Link>} /></section>}
  </div>;
}
