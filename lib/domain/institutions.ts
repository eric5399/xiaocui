...
export const INSTITUTION_CODES = INSTITUTIONS.map(({ code }) => code) as [
...
const institutionNames = new Map<string, string>(INSTITUTIONS.map(({ code, name }) => [code, name]));

export function institutionName(code: string | null | undefined): string {
  return code ? institutionNames.get(code) ?? "未知机构" : "未归类";
}
