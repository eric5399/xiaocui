...
begin
  ...
  select * into link from public.participant_access_links as access_link
    where access_link.token_hash = encode(extensions.digest(raw_token, 'sha256'), 'hex') for update;
  ...
  return query select task_row.id,task_row.invite_code from public.tasks as task_row where task_row.id=link.task_id;
end;
$$;
